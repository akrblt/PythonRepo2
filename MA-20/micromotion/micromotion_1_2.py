...
- Done:  Permettre de choisir un répertoire par une fenêtre
...
class ImageViewer(tk.Tk):
    def __init__(self,directory):
        ...
        self.image_files = [] # pour éviter bug si le répertoire n'existe pas.
        ...
        # Interface graphique ==================================================================================================
            ...
            # Bouton pour ouvrir la boîte de dialogue de sélection de répertoire
            self.load_dir = tk.Button(self.canvas_frame_cmd, text="dir", command=self.select_directory)
            self.load_dir.grid(row=0, column=0, padx=5, pady=10, sticky='e')
            ...
            self.auto_check.grid(row=1, column=1, padx=5, pady=10, sticky='w')
            # Images analysées
            self.label = tk.Label(self.canvas_frame_cmd, text="Images analysées:", font=("Arial", 8), height=2)
            self.label.grid(row=1,column=1,columnspan=3, padx=5,pady=10,sticky='w')
    ...
    def select_directory(self):
        ...
        selected_dir = filedialog.askdirectory(initialdir=self.directory_var.get())
        if selected_dir:
            # Obtient le répertoire de travail courant
            current_dir = os.getcwd()

            # Vérifie si les deux chemins sont sur le même disque (Windows)
            if os.path.splitdrive(selected_dir)[0].lower() == os.path.splitdrive(current_dir)[0].lower():
                # Convertit le chemin sélectionné en chemin relatif
                relative_path = os.path.relpath(selected_dir, current_dir)
                self.directory_var.set(relative_path)
            else:
                # Utilise le chemin absolu si les disques sont différents
