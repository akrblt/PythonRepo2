...
- Permettre de voir les 2 images originales dans le canvas (éviter de devoir avoir une autre fenêtre de visualisation)
- etc
'''
...
def compare_images(image1_path, image2_path, gray_mode, threshold,fact_reduc=2):
    ...
    print_time("Chargement 2 images " + str(sizex) + "p",start)

    # Appliquer un facteur de réduction
    start = time.perf_counter()
    if fact_reduc>1:
        img1 = cv2.resize(img1, (int(img1.shape[1]/fact_reduc), int(img1.shape[0]/fact_reduc)),interpolation=cv2.INTER_NEAREST)
        img2 = cv2.resize(img2, (int(img2.shape[1]/fact_reduc), int(img2.shape[0]/fact_reduc)),interpolation=cv2.INTER_NEAREST)
        print_time("Réduction facteur " + str(fact_reduc) ,start)
    ...
    print_time("Différence 2 imgages", start)
...
class ImageViewer(tk.Tk):
    def __init__(self,directory):
        ...
        # Interface graphique ==================================================================================================
            ...
            self.dir_entry = tk.Entry(self.canvas_frame_cmd, textvariable=self.directory_var, width=50)
            self.dir_entry.grid(row=0,column=1,columnspan=2, padx=10, pady=20,sticky='w')
            ...
            self.auto_check.grid(row=0, column=3, padx=10, pady=10, sticky='w')
            # Images analysées
            self.label = tk.Label(self.canvas_frame_cmd, text="Images analysées:", font=("Arial", 8), height=2)
            self.label.grid(row=1,column=0,columnspan=4, padx=10,pady=10,sticky='w')
            ...
            self.slider = tk.Scale(self.canvas_frame_cmd, from_=0, to=len(self.image_files) - 2, orient=tk.HORIZONTAL,length=380)
            self.slider.grid(row=2,column=1,columnspan=3, padx=10)

            # Cuseur de seuil
            self.label_tres = tk.Label(self.canvas_frame_cmd, text="seuil", font=("Arial", 8))
            self.label_tres.grid(row=3,column=0, padx=10)
            self.threshold_slider = tk.Scale(self.canvas_frame_cmd, from_=0, to=100, orient=tk.HORIZONTAL, length=380)
            self.threshold_slider.set(self.threshold)
            self.threshold_slider.grid(row=3,column=1,columnspan=3, padx=10)
            ...
            self.lst_fact_reduc.grid(row=4, column=1,  padx=10, pady=10, sticky='w')
            self.label_fact_reduc = tk.Label(self.canvas_frame_cmd, text="Facteur de réduction", font=("Arial", 8))
            self.label_fact_reduc.grid(row=4, column=0, padx=10, pady=10, sticky='e')
            ...
            self.lst_blob_size.grid(row=5, column=1, padx=10, pady=10, sticky='w')
            self.label_blob_size = tk.Label(self.canvas_frame_cmd, text="Taille taches (pixels)", font=("Arial", 8))
            self.label_blob_size.grid(row=5, column=0, padx=10, pady=10, sticky='e')
            ...
            self.lst_blob_min_percentage.grid(row=6, column=1, padx=10, pady=10, sticky='w')
            self.label_blob_min_percentage = tk.Label(self.canvas_frame_cmd, text="% différence min / tache", font=("Arial", 8))
            self.label_blob_min_percentage.grid(row=6, column=0, padx=10, pady=10, sticky='e')

            # Affichage du nombre de taches détectés
            self.label_nb_blob = tk.Label(self.canvas_frame_cmd, text="Nb taches", font=("Arial", 8))
            self.label_nb_blob.grid(row=7, column=0, columnspan=2,padx=10, pady=10, sticky='w')


            # Bouton fade (N/B clair)
            self.fade_button = tk.Button(self.canvas_frame_cmd, text="Atténuer image", command=self.toggle_fade)
            self.fade_button.grid(row=9,column=0, padx=10, pady=5,sticky='w')

            # Zone d'analyse
            self.cmd_edit_polygon=tk.Button(self.canvas_frame_cmd, text="Définir zone d'analyse (clic/clic droit)", command=self.edit_polygon)
            self.cmd_edit_polygon.grid(row=9, column=1, columnspan=3,padx=10, pady=5, sticky='w')
    ...
    def update_image(self):
        ...
        img1 = self.image_files[self.current_index]
        img2 = self.image_files[self.current_index + 1]

        print()
        print("Traitement de",img1,img2,"facteur de différence",self.threshold,"%", "taille des taches",self.blob_size,"pixels", "pourcentage de taches",self.blob_min_percentage*100,"%")

        # Comparer les images et créer l'image de différence
        base_image, self.diff_image, self.img_sizex = compare_images(img1, img2, self.gray_mode, self.threshold,self.fact_reduc)
        ...
        print_time("Détection taches :" + str(self.blob_size) + "/" + str(self.blob_min_percentage),start)
        
        #agglomération des taches
        start = time.perf_counter()
        if(len(self.mvt_blobs)<1000):
            self.mvt_blob=agglomerate(self.mvt_blobs)
        print_time("Agglomération des taches :",start)
    ...
    def display_blobs(self):
       if (self.zoom_factor>=1):
            factor=self.img_sizex/self.width/self.fact_reduc/self.zoom_factor
