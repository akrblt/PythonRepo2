...
def generate_sql_from_text():
    ...
    for li in range(1,len(lines)) :
        data_rows.append(lines[li].split(";"))

    # traitement de chaque ligne
    for row in data_rows:
        if len(row) != len(headers): # on laisse tomber les lignes de mauvaise longueur avec un warning
            messagebox.showwarning("Attention",
                                   "Une ligne de données ne correspond pas au nombre de colonnes des en-têtes.")
        else:
            query = f"INSERT INTO {table_name} ({', '.join(headers)}) \n VALUES ("
            # création des VALUES (avec traitement différencié pour NULL et les nombres)
            for value in row:
                print(value, is_number(value))
                if value == "NULL" or is_number(value) :
                    query += f"{value},"
                else :
                    query += f"'{value}',"
    ...
    sql_widget.delete("1.0", tk.END)
    sql_widget.insert(tk.END, "\n".join(sql_queries))
    messagebox.showinfo("Succès", "Requêtes SQL générées avec succès.")
